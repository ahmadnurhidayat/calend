...
export default function EventTypeBookingPage({
    ...
    useEffect(() => {
        if (!selectedDate || !username || !eventSlug) return;

        const fetchSlots = async () => {
            const dateStr = selectedDate.toISOString().split('T')[0];
            try {
                const res = await fetch(
                    `/api/slots?username=${username}&eventSlug=${eventSlug}&date=${dateStr}&timezone=${candidateTimezone}`
                );
        ...
        fetchSlots();
    }, [selectedDate, candidateTimezone, username, eventSlug]);

    const handleBooking = async () => {
        if (!host || !eventType || !selectedDate || !selectedTime || !candidateName || !candidateEmail) {
        ...
        return (
            <div className="flex-1 flex items-center justify-center p-4">
                <div className="text-center">
                    ...
                    <p className="text-muted">This event type does not exist or is no longer active.</p>
                </div>
            </div>
        );
    }
            ...
            <div className="flex-1 flex items-center justify-center p-4">
                <div className="glass-card p-8 max-w-md text-center">
                    ...
                    <p className="text-muted mb-4">
                        Your {eventType.duration}-minute {eventType.title} with {host.name} has been scheduled.
                    </p>
        ...
        <div className="p-4 sm:p-8">
            <div className="max-w-2xl mx-auto">
                <div className="text-center mb-8">
                    ...
                    <p className="text-muted">{host.name} &middot; {eventType.duration} minutes</p>
